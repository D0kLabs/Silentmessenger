    ...
    implements IAuthenticatedMode
{
  ...
  {
    byte[] nonce = (byte[]) attrib.get(IMode.IV);
    if (nonce == null)
      throw new IllegalArgumentException("no nonce provided");
    byte[] key = (byte[]) attrib.get(IBlockCipher.KEY_MATERIAL);
    if (key == null)
      throw new IllegalArgumentException("no key provided");

    Arrays.fill(t_n, (byte) 0);
    nonceOmac.reset();
    nonceOmac.init(Collections.singletonMap(IMac.MAC_KEY_MATERIAL, key));
    ...
    t_n[t_n.length - 1] = 1;
    headerOmac.reset();
    headerOmac.init(Collections.singletonMap(IMac.MAC_KEY_MATERIAL, key));
    headerOmac.update(t_n, 0, t_n.length);
    t_n[t_n.length - 1] = 2;
    msgOmac.reset();
    msgOmac.init(Collections.singletonMap(IMac.MAC_KEY_MATERIAL, key));
    msgOmac.update(t_n, 0, t_n.length);
    Integer modeSize = (Integer) attrib.get(IMode.MODE_BLOCK_SIZE);
    if (modeSize == null)
      modeSize = Integer.valueOf(cipherBlockSize);
    HashMap ctrAttr = new HashMap();
    ctrAttr.put(IBlockCipher.KEY_MATERIAL, key);
    ctrAttr.put(IMode.IV, N);
    ctrAttr.put(IMode.STATE, Integer.valueOf(IMode.ENCRYPTION));
    ctrAttr.put(IMode.MODE_BLOCK_SIZE, modeSize);
    ctr.reset();
    ctr.init(ctrAttr);
    Integer st = (Integer) attrib.get(IMode.STATE);
    if (st != null)
      {
        state = st.intValue();
        if (state != IMode.ENCRYPTION && state != IMode.DECRYPTION)
          throw new IllegalArgumentException("invalid state");
      }
    else
      state = IMode.ENCRYPTION;

    Integer ts = (Integer) attrib.get(IMac.TRUNCATED_SIZE);
    if (ts != null)
      tagSize = ts.intValue();
    else
