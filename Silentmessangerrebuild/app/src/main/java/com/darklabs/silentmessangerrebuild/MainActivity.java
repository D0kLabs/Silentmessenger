...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mTextView = (TextView) findViewById(R.id.message_view); //WTF?
                        ...
                        public void onSuccess() {
                            mWifiP2pManager.setServiceResponseListener(mChannel, new WifiP2pManager.ServiceResponseListener() {
                                ...
                                public void onServiceAvailable(int i, byte[] bytes, WifiP2pDevice wifiP2pDevice) {
                                    ...
                                    byte[] bQeury = null;
                                    try {
                                        bQeury = queryData.getBytes("UTF-8");
    ...
    public class mWiFiDirectBroadcastReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (mWifiP2pManager.WIFI_P2P_STATE_CHANGED_ACTION.equals(resiver)) {
                int state = intent.getIntExtra(WifiP2pManager.EXTRA_WIFI_STATE, -1);
                if (state == WifiP2pManager.WIFI_P2P_STATE_ENABLED) {
                    //Toast something)
                }
            } else if (mWifiP2pManager.WIFI_P2P_PEERS_CHANGED_ACTION.equals(resiver)) {
                mWifiP2pManager.requestPeers(mChannel, peerListListener);
            } else if (mWifiP2pManager.WIFI_P2P_CONNECTION_CHANGED_ACTION.equals(resiver)) {
                //AHTUNG!!! Connection FAILS!
            } else if (mWifiP2pManager.WIFI_P2P_THIS_DEVICE_CHANGED_ACTION.equals(resiver)) {
